...
export default function Sidebar({ activeTab, setActiveTab, hasVideo }) {
  const tabs = [
    ...
    { id: 'intel', label: 'TORRENT INTEL' },
    { id: 'matrix', label: 'JOB MATRIX' },
  ];

  if (hasVideo) {
    tabs.push({ id: 'video', label: 'VIDEO PLAYER' });
  }
  ...
  return (
    <aside className="w-full lg:w-[220px] shrink-0 border-r-0 lg:border-r border-[var(--line)] bg-[#07090a]/40 flex flex-col font-mono">
      <div className="p-4 border-b border-[var(--line)] hidden lg:block">
        <span className="text-[0.7rem] text-[var(--muted)] uppercase tracking-widest block mb-1">Navigation</span>
        <h3 className="text-sm font-bold  text-[var(--acid)] m-0">SWARM CONTROL</h3>
      </div>
